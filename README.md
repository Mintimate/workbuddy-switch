...
| 自动签到 | 默认开启；支持按账号关闭；可设置签到时间段，窗口内随机时刻自动签到（需 App 在窗口附近运行）；一键全部签到；30 天签到日志 |
...
3. **自动签到**：账号页可直接切换全局开关；账号卡片「更多操作 → 允许自动签到」可单独排除账号，重启后保留。关闭后，后台与账号页不再自动查询或签到；刷新积分时也会忽略该账号的签到并提示数量，积分照常刷新。单账号「手动签到」及设置页、托盘的「全部立即签到」仍可主动执行；设置页可调整保活参数、立即签到并查看日志
