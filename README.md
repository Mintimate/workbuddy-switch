...
8. **VS Code CodeBuddy 扩展**：账号卡片可一键把所选账号写入 VS Code 内的 CodeBuddy 扩展（`tencent-cloud.coding-copilot`，与 CN IDE 同源 www.codebuddy.cn）。切换前请**完全退出 VS Code**（运行中写入会被覆盖且不会生效），切换后重新打开并重载窗口生效；首次使用前请先在 VS Code 中安装并登录一次该扩展，以生成系统凭据存储。与 CodeBuddy CN IDE、CodeBuddy CLI 相互独立。
