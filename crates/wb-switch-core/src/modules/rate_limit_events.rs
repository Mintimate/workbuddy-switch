...
//! 恢复时刻（`limits::parse_reset_at`）→ 实时归因「当前账号 = 事件账号」→ 入账（内存 + 落盘）
//! → 由宿主 emit `rate-limits-updated` 通知前端。
//!
//! 关键决定（实施时定的开放点）：
...
//!
//! 事件 payload 不含账号 uid，归因完全依赖「hook 触发时该客户端当前登录的是谁」，
//! 因此必须在事件到达时归因，不能延后。
...
/// 实时归因：hook 在 429 当轮触发，所以「该客户端此刻登录的账号」就是事件账号。
///
/// - CLI：轮换状态文件的 `activeAccountId`（wb-switch 切换时写入）；
/// - WorkBuddy：① 登录态文件 uid → 账号库；② 会话 id 查 `sessions` 表兜底；
/// - 都拿不到 → 丢弃（宁可少显示，不可显示错账号）。
fn attribute(event: &QuotaEvent, source: HookSource, ctx: &IngestContext) -> Option<String> {
    match source {
        HookSource::Cli => {
            let account_id = cli_active_account_id(&ctx.cli_state_path)?;
...
fn ingest_lines(ctx: &IngestContext, state: &mut State, lines: &[String], now: i64) -> bool {
    ...
    for line in lines {
        ...
        let Some(account_id) = attribute(&event, source, ctx) else {
            continue;
        };
        merge_entry(
            &mut state.events,
...
mod tests {
    ...
    /// CLI 的当前账号不在账号库（用户删了账号 / 状态文件属于别的库）→ 丢弃，不误归。
    #[test]
